...
# Route for displaying library loans
@app.route('/library_loans_index', methods=['GET', 'POST'])
def library_loans_index():
    # Connect to the database
    conn = get_db()
    cursor = conn.cursor()
    ...
    if request.method == 'POST':
        ...
        cursor.execute('''
            ...
            WHERE LoanID LIKE ? OR ISBN LIKE ? OR MemberID LIKE ? OR LoanDate LIKE ? OR ReturnDate LIKE ?
        ''', ('%' + search_query + '%', '%' + search_query + '%', '%' + search_query + '%', '%' + search_query + '%', '%' + search_query + '%'))
        loans = cursor.fetchall()
    else:
        # Retrieve all book loans from the BookLoan table with book and member details
    ...
    # Render the template with the list of book loans
    return render_template('library_loans_index.html', loans=loans)
